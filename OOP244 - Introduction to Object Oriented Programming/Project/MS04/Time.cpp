/*
 ...
 2024/3/16  Add {} behind the char buf[MAX_DIGITS_MINS] to initialize all elements of the array to '\0'
 2024/3/23  change from  char buf[MAX_DIGITS_MINS]{}; to char buf[6]{}; 
			change from istr >> buf; to istr.get(buf, 6); to not read the rest of the line  
 -----------------------------------------------------------
...
namespace seneca
{
	...
	istream& Time::readMethod(istream& istr)
	{
        ...
        char buf[6]{}; // 23:45 (5 characters + 1 null terminator)
		istr.get(buf, 6);
		if (buf[0] != '\0')
		{
