...
int LoadShipmentOntoTruck(struct Truck* truck, struct Shipments shipment) {
    ...
    {
        if (truck->capacityPercentage + (shipment.weight / MAX_CAPACITY) * 100 < 100 && (truck->volumePercentage + (shipment.volume / MAX_VOLUME) * 100 < 100) )
        {
...
int FindNearestTruck(const struct Map* map, struct Point deliveryPoint, struct Truck* trucks, int numTrucks)
{
    ...
    if (truckIndex != -1)
    {
        for (int i = 0 ; i < currentShortestPath.numPoints; i++)
		{
			addPtToRoute(&trucks[truckIndex].DeliveryPath, currentShortestPath.points[i]);
		}
    }
    return truckIndex;
}
