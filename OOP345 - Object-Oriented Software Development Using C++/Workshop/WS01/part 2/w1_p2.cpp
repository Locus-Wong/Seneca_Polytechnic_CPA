...
int main(int argc, char* argv[])
{
	...
	seneca::FoodOrder recordedDeliveryOrders[6]; // default constructor
	// Keep a count of how many orders recorded
	size_t numDeliveries = 0;

	seneca::FoodOrder currentOrder; // default constructor
	...
	for (auto day = 1; day < argc; ++day)
	{
		...
		while (in)
		{
			...
			if (ordertag == 'I')
			{
				seneca::FoodOrder copy = currentOrder; // copy constructor
				copy.display();
			}
			else if (ordertag == 'D')
			{ // adds the delivery orders to the record
				recordedDeliveryOrders[numDeliveries++] = currentOrder; // copy assignment
